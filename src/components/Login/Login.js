...
const Login = (props) => {
  ...
  const [collgeState, dispatchCollege] = useReducer(collegeReducer, {
    value: "",
    isValid: null,
  });
  ...
  const { isValid: collegeIsValid } = collgeState;
  const { isValid: passwordIsValid } = passwordState;

  useEffect(() => {
    const identifier = setTimeout(() => {
      console.log("Checking form validity");
      setFormIsValid(emailIsValid && passwordIsValid && passwordIsValid);
    }, 500);
  ...
  const submitHandler = (event) => {
    ...
    if (formIsValid) {
      authCtx.onLogin(emailState.value, collgeState.value, passwordState.value);
